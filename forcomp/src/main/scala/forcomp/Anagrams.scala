...
object Anagrams {
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = w.groupBy(c => c.toLower).mapValues(_.length).toList.sortBy(_._1)

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = s.foldLeft(List[(Char, Int)]())((s, i) => {
    val map1 = s.toMap
    val map2 = wordOccurrences(i).toMap
  ...
  /** The `dictionaryByOccurrences` is a `Map` from different occurrences to a sequence of all
    ...
    *
    */
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = dictionary.foldLeft(Map[Occurrences, List[Word]]())((s, i) => {
    val ocr = wordOccurrences(i)
    s.updated(ocr, i :: s.getOrElse(ocr, List()))
  })
